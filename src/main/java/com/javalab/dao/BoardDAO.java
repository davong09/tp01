...
public class BoardDAO {
	...
	public int insertBoard(BoardVO boardVO) {
		...
		try{
			...
			//String sql = "Insert into board (bno, title, content, member_id, reply_group, reply_order, reply_indent, file_name)  ";
			//sql += 		" values (?,?,?,?,?,0,0, ?) ";	// 맨뒤 ? fileName
			
			// 2. 첨부 파일의 유무에 따른 쿼리문 변경
			StringBuilder sql = new StringBuilder();
	        sql.append("Insert into board (bno, title, content, member_id, rating, reply_group, reply_order, reply_indent");
...
//			sql.append("where a.row_num between ? and ?");
	...
	public BoardVO getBoard(int bno) {
		...
		try {
	        ...
	        // 게시물 조회 쿼리
	        String sql = "select bno, title, content, member_id, rating, reg_date, hit_no, " +
	        			"file_name " +	        		
